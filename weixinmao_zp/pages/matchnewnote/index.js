...
Page({
  ...
  gethouselist: function (e) {
   ...
   // var cityid = wx.getStorageSync('cityinfo').id;
    ...
    app.util.request({
      'url': 'entry/wxapp/Getmatchnewnotelist',
      data: { sessionid: userinfo.sessionid, uid: userinfo.memberInfo.uid, page: that.data.page, houseareaid: that.data.houseareaid, housepriceid: that.data.housepriceid, worktype: that.data.worktype, companyid: companyid },
      success: function (res) {
        if (!res.data.message.errno) {
