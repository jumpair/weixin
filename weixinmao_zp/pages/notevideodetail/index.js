...
Page({
  ...
  onLoad: function (e) {
    ...
    this.noteVideo = wx.createVideoContext('noteVideo')
  ...
  //  var showcontact = true;
    ...
    app.util.request({
      ...
      success: function (res) {
        ...
        that.data.truetel = res.data.data.workerdetail.tel2;
          wx.setNavigationBarTitle({
  ...
  toLookUser: function () {
    ...
    app.util.request({
      ...
      data: { id: that.data.id, sessionid: userinfo.sessionid, uid: userinfo.memberInfo.uid,notetype:1 },
      success: function (res) {
        if (!res.data.message.errno) {
          ...
          if (res.data.data.status == 1) {
            ...
            wx.makePhoneCall({
              phoneNumber: that.data.truetel, //此号码并非真实电话号码，仅用于测试
              success: function () {
                console.log("拨打电话成功！")
              },
              fail: function () {
  ...
  toLookContact: function (e) {
    var that = this;
    console.log('ssssssssqqqqqqqqqqq');
    ...
    if (!companyid) {
      ...
      app.util.request({
        ...
        data: { id: that.data.id, companyid: companyid,notetype:1 },
        success: function (res) {
          if (!res.data.message.errno) {
  ...
  toLookContact2:function(e){
    ...
    if (!companyid) {
      ...
      app.util.request({
        'url': 'entry/wxapp/CheckLookrecord',
        data: { id: that.data.id, companyid: companyid },
        success: function (res) {
          if (!res.data.message.errno) {
