...
Page({
  ...
  onLoad: function (options) {
    ...
    var company_id = parseInt(uid_array[2]);
    
    that.data.id = id;
    that.data.tid = tid;
    that.data.companyid = company_id;
    wx.setStorageSync('tid', tid);
    wx.setStorageSync('company_id', company_id);
   ...
   // var shareid = that.data.shareid;
    ...
    if (appuser) {
      if (appuser.hasOwnProperty("wxInfo")) {
        ...
        var data = { id: that.data.id, uid: appuser.memberInfo.uid };

      } else {
        that.data.isuser = false;
        var data = { id: that.data.id, uid: 0 };
  ...
  , doSendjob: function (e) {
    ...
    app.util.request({
      'url': 'entry/wxapp/sendcompanyjob',
      data: { form_id: form_id, companyid: companyid, jobid: jobid, sessionid: userinfo.sessionid, uid: userinfo.memberInfo.uid },
      success: function (res) {
