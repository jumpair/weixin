...
Page({
  ...
  var city = wx.getStorageSync('city');
      ...
      success: function (res) {
        if (!res.data.message.errno) {
          ...
          if (res.data.data.intro.ischeck == 1){
            ...
            wx.setNavigationBarTitle({
              title: selecttitle[3] + wx.getStorageSync('companyinfo').name,
            })
    ...
    app.util.request({
      'url': 'entry/wxapp/getmatchjob',
      data: { sessionid: userinfo.sessionid, uid: userinfo.memberInfo.uid, cityid: cityid, page: that.data.page, houseareaid: that.data.houseareaid, housepriceid: that.data.housepriceid, housetype: that.data.housetype, latitude: latitude, longitude: longitude },
      success: function (res) {
        if (!res.data.message.errno) {
          that.setData({
